...
The admin-only `/admin/announcements` page sends Discord embeds through `DISCORD_BOT_TOKEN` (raw token only, without the `Bot ` prefix). When `DISCORD_GUILD_ID` is set, the page can load text channels into a dropdown; `DISCORD_ANNOUNCEMENT_CHANNEL_ID` only pre-fills a default channel and can stay empty.
## Deploy on Vercel
